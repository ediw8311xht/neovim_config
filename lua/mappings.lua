...
LEADER_MAPPINGS = {
  n = {
    ...
    --[[ --]]
    B     = { group="buffer"},
    Bc    = { desc='copy buffer to clip',       default='gg"+yG<c-o>'},
    Bd    = { desc='delete buffer',             cmd='bd'},
    Bn    = { desc='new buffer',                cmd='enew'},
    ...
    Fsa   = { desc='[args] session save',       default=':AutoSession save '},
    ...
    H     = { group="help"},
    HM    = { desc='print all mappings',        lua_call='GetMappings()'},
    Hg    = { desc='helpgrep',                  default=':vert helpgrep '},
    Hh    = { desc='get highlight',             lua_call='GetHL()'},
    Hk    = { desc='vim help tags',             cmd='Telescope help_tags'},
    Hm    = { desc='open mappings file',        cmd='edit ' .. vim.g.mapping_file},
    Ho    = { desc='vim options',               cmd='Telescope vim_options'},
    Ht    = { desc='telescope commands',        cmd='Telescope commands'},
    I     = { group="lsp"},
    IA    = { desc='stop lsp',                  cmd='lsp stop'},
    Ia    = { desc='start lsp',                 cmd='lsp enable'}, -- this sucks need to find way to start only those defined by nvim lsp config
    Ir    = { desc='restart lsp',               cmd='lsp restart'},
    Ic    = { desc='show code-action',          lua_call='require("actions-preview").code_actions()'},
    Id    = { desc='go to definition',          lua_call='vim.lsp.buf.definition()'},
    Ihc   = { desc='clear highlight symbol',    lua_call='vim.lsp.buf.clear_references()'},
    Ihs   = { desc='highlight symbol',          lua_call='LspDocumentHighlight()'},
    ...
    In    = { desc='goto next error',           lua_call='vim.diagnostic.goto_next()'},
    Ip    = { desc='goto prev error',           lua_call='vim.diagnostic.goto_prev()'},
    Iy    = { desc='[!]lsp_lines',              lua_call='require("lsp_lines").toggle()'},
    M     = { group="misc"},
    ...
    S     = { group="execution"},
    SX    = { desc='execute with args',         default=':!%'},
    Sl    = { desc='luafile',                   cmd='luafile %'},
    Ss    = { desc='source file',               cmd='%so'},
    Sx    = { desc='execute',                   cmd='!%'},
    T     = { group="treesitter"},
    Tl    = { desc='get treesitter parser',     cmd='lua= vim.treesitter.get_parser(0):lang()'},
    Ts    = { desc='treesitter status',         cmd='lua= MyTreesitterStatus()'},
    Tt    = { desc='open treesitter tree',      cmd='lua vim.treesitter.inspect_tree()'},
    U     = { desc='lf cd',                     cmd='Lfcd'},
    W     = { desc='write',                     default=':w<ESC>'},
    b     = { desc='open buffer',               default=my_buffers },
    c     = { group="editor setting"},
    cA    = { desc='autofold comments OFF',     cmd='AutoFoldComments off'},
    cB    = { desc='prev background',           vim_command='CyBack(-1)'},
    cC    = { desc='[!]folds in gutter',        vim_command='TogFoldColumn()'},
    cJ    = { desc='prev scheme',               vim_command='SetColScheme(-1)'},
    cL    = { desc='cycle statusline',          cmd='lua= Cycle("statusline", vim.g.my_statuslines)'},
    cS    = { desc='[!]spell',                  cmd='set spell!'},
    ca    = { desc='autofold comments ON',      cmd='AutoFoldComments multi'},
    cb    = { desc='next background',           vim_command='CyBack(+1)'},
    cc    = { desc='[!]line length indicator',  vim_command='TogColorColumn()'},
    ...
    ch    = { desc='format2',                   lua_call='vim.lsp.buf.format()' },
    cj    = { desc='next scheme',               vim_command='SetColScheme(+1)'},
    ck    = { desc='correctcolors()',           cmd='lua CorrectColors()'},
    cl    = { desc='[!]cursorline',             cmd='lua ToggleHighlight({"CursorLine"})'},
    cr    = { desc='[!]rainbow',                vim_command='rainbow_delimiters#toggle(0)'},
    cs    = { desc='[!]statusline',             vim_command='TogLastStatus()'},
    cv    = { desc='[!]virtualedit',            vim_command='TogVirtualEdit()'},
    cw    = { desc='[!]wrap',                   cmd='set wrap!'},
    df    = { desc='find space eol',            cmd='%s/\\s\\+\\ze$//gc'},
    grg   = { desc='search in all buffers',     default=my_live_grep},
    i     = { desc='show diagnostics',          lua_call='vim.diagnostic.open_float(nil, {focus=T, scope="cursor"})'},
    s     = { desc='switch pane',               default='<C-w><C-p>'},
    u     = { desc='lf file manager',           cmd='Lf'},
    vt    = { desc='title case',                cmd='s/\\v\\c\\w(\\a*(\'\\a{0,1})?\\w)?/\\u\\0/g | nohl'},
    wj    = { desc='decrease size split',       default='40<c-w><'},
    wk    = { desc='increase size split',       default='40<c-w>>'},
    wo    = { desc='toggle fullscreen',         cmd='ToggleFullscreen'},
    x     = { desc='execute',                   cmd='!%:p'},
    y     = { desc='bg transparent',            cmd='hi Normal guibg=Transparent'},
    z     = { desc='',                          default='z'},
  ...
  v = {
    vc     = { desc='column',          default=':%!column -o " " -t<ESC>'},
    vmf    = { desc='bc [math] float', default=':!bc -l<ESC>'},
    vmi    = { desc='bc [math] int',   default=':!bc -l<ESC>'},
    vmq    = { desc='qalc [math]',     default=':!xargs qalc --color=never --terse<ESC>'},
...
-- get region 			      
-- PERSONAL_MAPPINGS = { desc=["regular"] = REGULAR_MAPPINGS, ["leader"] = LEADER_MAPPINGS }
